...
Mediation of negative effects in the Northeast, but strong effects crop up in Center City.

## Results: Geographic Heterogeneity
```{r, echo = FALSE}
par(mfrow=c(2, 3), mar = c(0, 0, 1, 0),
    oma=c(5.1, 4.1, 4.1, 1.1))
rng <- 1.03 * phila_azav_quad@
  data[ , range(.SD[ , grep("ever_paid", names(phila_azav_quad@data)),
                     with = FALSE])]
ys <- unlist(transpose(lapply("ever_paid_" %+% mos, paste0, "_"%+%trt.nms[-1])))
yls <- unlist(transpose(lapply("ever_paid_" %+% mos %+% "_low",
                               paste0, "_"%+%trt.nms[-1])))
yus <- unlist(transpose(lapply("ever_paid_" %+% mos %+% "_high",
                               paste0, "_"%+%trt.nms[-1])))
for (ii in (qs <- phila_azav_quad@data[ , unique(quadrant)])){
  phila_azav_quad@data[quadrant==ii,{
    bpx <- barplot(matrix(unlist(mget(ys)), nrow = 3), beside = TRUE,
                 col = get.col(gsub(".*_", "", ys)),
                 density = 20 * 3 ^ (0:2), ylim = rng, yaxt = "n")
    tile.axes(match(ii, qs), 2, 3, use.x = FALSE,
          params = list(list(), y = list(las = 2)))
    text(bpx, 0, labels = rep(c("J","S","D"), 6))
    title(ii)
    arrows(bpx, unlist(mget(yls)), 
           bpx, unlist(mget(yus)), angle = 90,
           code = 3,lwd = .5, length = .01, lty = 1)}]
  box()
}
