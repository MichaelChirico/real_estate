...
## Sample Description: Outcomes
```{r third, echo = FALSE, results = "asis"}
<<descriptives_2>>
```
...
## Sanity Check: Balance on Observables
```{r fifth, echo = FALSE}
#since data.table j can't recognize output as invisible,
#  'barplot' call plops out some noise; wrap in invisible
#  to suppress this.
invisible(
properties[(!holdout),
           #count properties (.N) and owners (uniqueN(owner1))
           .(.N, ownN = uniqueN(owner1)), keyby = treat7
           ][,{
             layout(mat = matrix(1:2), heights = c(.8, .2))
             par(mar = c(1, 4.1, 4.1, 2.1))
             x <- barplot(c(rbind(N, ownN)), cex.main = 1.5,
                          names.arg = rep(treat7, each = 2L), las = 2L,
                          main = "Numbers of Properties and Owners\nBy Treatment",
                          col = get.col(rep(trt.nms, each = 2L)),
                          density = rep(c(-1L, 20L), .N), cex.names = .75,
                          ylim = c(0, 1.3 * max(N)))
             par(mar = rep(0, 4))
             plot(0, 0, type = "n", ann = FALSE,
                  axes = FALSE, xlim = range(x))
             legend(max(x)/2, 0, bty = "n", xjust = .3,
                    legend = c("# Properties", "# Owners"),
                    density = c(-1, 20), horiz = TRUE, text.width = 5)}]
)
```
