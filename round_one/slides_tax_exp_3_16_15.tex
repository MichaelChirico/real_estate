...
\item Property tax compliance is a significant problem in many
large U.S. cities. Philadelphia is a prominent example.
...
\item Tax payers may comply with tax laws because of deeply ingrained social norms. In particular, tax payers may comply with the tax laws due to peer pressure and the desire to conform
...
\item  In the Adreoni model, the pay-offs of both tax payers are:
\begin{eqnarray*}
...
\item Of course, these interventions face larger legal hurdles and are, therefore, harder to implement. 
\item The basic idea behind our peer conformity treatment is to reinforce the notion
that one's peers are paying taxes.
\end{itemize}
}
...
these bills and returns the missives to DoR, which then processes them
...
subset of bills made their way back to DoR to check first-hand the extent of
treatment fidelity. 
...
\begin{itemize}
\item  Unlike several recent papers (Kleven et al. 2011; Slemrod, Blumenthal, and Christian 2001), which have found large increases in compliance after providing information about the threat of auditing, we find no evidence of a deterrence effect. 
...
\item It is difficult to aver that one could significantly alter perceptions of beliefs of punishment by sending one letter in a city with already-high property tax delinquency. 
